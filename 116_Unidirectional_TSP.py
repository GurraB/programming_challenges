...
def rec(pos, matrix, savedValues, savedPaths):
    ...
    '''if upRes == rightRes == downRes:
        returnVal = (upRes, (min([upPos[0], rightPos[0], downPos[0]]), upPos[1]))
    elif upRes == rightRes:
        returnVal = (upRes, (min([upPos[0], rightPos[0]]), upPos[1]))
    elif upRes == downRes:
        returnVal = (upRes, (min([upPos[0], downPos[0]]), upPos[1]))
    elif rightRes == downRes:
        returnVal = (rightRes, (min([rightPos[0], downPos[0]]), rightPos[1]))
    else:
        returnVal = min([(upRes, upPos), (rightRes, rightPos), (downRes, downPos)], key=lambda item: (item[0], item[1][0]))
    '''
    returnVal = min([(upRes, upPos), (rightRes, rightPos), (downRes, downPos)], key=lambda item: (item[0], item[1][0]))
    #returnVal = min([(rightRes, rightPos), (upRes, upPos), (downRes, downPos)], key=lambda item: item[0])
